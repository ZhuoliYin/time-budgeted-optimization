...
class Tester:
    ...
    def _test(self, env_class, compute_gap=False):
        ...
        while episode < test_num_episode:
            ...
            # compute_gap = False
            if compute_gap:
                if self.args.problem in ["OPTWVP", "SOPTWVP", "TOPTWVP"]:
                    grid_factor = -0.001
                else:
                    grid_factor = 1.
                ...
                opt_sol = [i[0] / grid_factor for i in opt_sol]
    ...
    def _test_one_batch(self, test_data, env):
        ...
        while not done:
            selected, prob, service_time_normed = self.model(state, pomo=self.env_params["pomo_start"],
                                                      tw_end=env.node_tw_end if self.args.problem in ["OPTWVP", "TOPTW", "OPTW", "TSPTW"] else None,
                                                      no_sigmoid=True)
            state, reward, done, infeasible = env.step(selected,
                                                         service_time_normed = service_time_normed,
                                                    )  
        aug_reward = reward.reshape(aug_factor * sample_size, int(env.batch_size/aug_factor/sample_size), env.pomo_size)

        infeasible = infeasible.reshape(aug_factor * sample_size, int(env.batch_size/aug_factor/sample_size), env.pomo_size)  # shape: (augmentation, batch, pomo)
        ...
        if self.tester_params["fsb_dist_only"]:
            ...
            if tour_path is not None:
                ...
                route_list = env.selected_node_list.reshape(aug_factor * sample_size, int(env.batch_size/aug_factor/sample_size), -1)
